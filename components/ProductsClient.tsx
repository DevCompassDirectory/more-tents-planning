  ...
  function addReq() {
    setEdit((prev) =>
      prev
        ? {
            ...prev,
            requirements: [...prev.requirements, { naam: "", aantal: 1 }],
          }
        : prev
    );
  }
  ...
  return (
    <div>
      <div className="mb-6 flex justify-between items-center gap-3 flex-wrap">
        <h1 className="font-display text-2xl text-charcoal-900">Producten</h1>
        <button
          type="button"
        ...
        <div className="p-6">
          {edit ? (
            ...
            <div className="text-center text-charcoal-900/60 py-12 text-sm">
              Geen product geselecteerd. Klik op + Nieuw product om te
              beginnen.
            </div>
          )}
        </div>
      ...
      {product.requirements.length === 0 ? (
        ...
        <div className="space-y-1">
          {product.requirements.map((r) => (
            ...
            >
              <span>{r.naam}</span>
              <span className="font-medium">{formatNum(r.aantal)}×</span>
            </div>
          ))}
        </div>
      )}
...
function ProductForm({
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-[1fr_140px_140px] gap-3 mb-3">
        ...
        <Field label="Categorie">
          <select
            ...
            onChange={(e) =>
              onPatch({ categorie: e.target.value as LineItemCategorie })
            }
            className={inputCls}
          >
      ...
      <Field label="Notities (optioneel)" className="mb-5">
        <textarea
          value={edit.notities}
      ...
      <div className="space-y-2 mb-3">
        {edit.requirements.map((r, idx) => (
          ...
          >
            <input
              value={r.naam}
              onChange={(e) => onPatchReq(idx, { naam: e.target.value })}
            ...
            <input
              ...
              value={r.aantal}
              onChange={(e) =>
                onPatchReq(idx, { aantal: parseFloat(e.target.value) || 0 })
              }
